...
import emailPassword from "../helpers/emailOlvidePassword.js";
...
const autenticar = async (req,res) => {
   ...
   if( await usuario.comprobarPassword(password)){
      ...
      res.json({
        ...
        email: usuario.email,
        token: generarJsonWebToken(usuario.id),
      })
   }else{
...
const comprobarToken = async (req, res) => {
    ...
    if(tokenValido){
        //El token y usuario son validos
...
const nuevoPassword = async (req, res) => {
   ...
   try {
       ...
       res.json({ msg: 'Password cambió correctamente'})
   } catch (error) {
      console.log(error)
   }
}
...
const actualizarPassword = async (req, res) => {
    ...
    if( await veterinario.comprobarPassword(pwd_actual)){
        ...
        await veterinario.save()
        return res.json({msg : 'Passwor Almacenado correctamente'})
    }else{ 
